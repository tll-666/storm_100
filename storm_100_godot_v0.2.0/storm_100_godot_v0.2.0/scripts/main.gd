...
func _on_choice_selected(index: int) -> void:
	...
	if action_type == "travel_to_store":
		...
		if index == 0:
			_enter_supermarket()
		return
	if action_type == "shop_shelf":
		_handle_shop_shelf_choice(index)
		return
	if action_type == "checkout":
		_handle_checkout_choice(index)
		return
...
func _open_shop_shelf(shelf_id: String) -> void:
	...
	for item_id in item_ids:
		...
		choices.append(
			"%s ¥%d（%d格）"
			% [str(item.get("name", item_id)), int(item.get("price", 0)), int(item.get("slots", 1))]
		)
	choices.append("先不拿")
	pending_action = {"type": "shop_shelf", "shelf_id": shelf_id, "item_ids": item_ids}
	hud.show_dialogue(
		str(shelf.get("name", "货架")),
		"%s\n\n当前：%d/%d格，预计¥%d。"
		% [str(shelf.get("text", "")), GameState.cart_slots(), GameState.trunk_capacity, GameState.cart_total()],
		choices
	)


func _handle_shop_shelf_choice(index: int) -> void:
	var item_ids: Array[String] = []
	item_ids.assign(pending_action.get("item_ids", []))
	...
	if not error.is_empty():
		...
		var item := GameState.shop_item(item_id)
		hud.show_toast("已拿取：%s" % str(item.get("name", item_id)))
	_open_shop_shelf(shelf_id)
	_update_hud()


func _open_checkout() -> void:
	var summary := GameState.cart_summary()
	var text := "%s\n\n合计¥%d，占用%d/%d格。"
	text = text % [summary, GameState.cart_total(), GameState.cart_slots(), GameState.trunk_capacity]
	var choices: Array[String] = []
	if GameState.shopping_cart.is_empty():
		text += "\n你也可以决定这次什么都不买。"
		choices = ["不购物，直接回家", "继续看看"]
	else:
		choices = ["结账回家", "拿出最后一件", "继续购物"]
	pending_action = {"type": "checkout"}
	hud.show_dialogue("收银台", text, choices)


func _handle_checkout_choice(index: int) -> void:
	if GameState.shopping_cart.is_empty():
		if index == 0:
	...
	if index == 0:
		...
		return
	if index == 1:
		var removed_id := GameState.remove_last_shop_item()
		var removed := GameState.shop_item(removed_id)
		hud.show_toast("已放回：%s" % str(removed.get("name", removed_id)))
		_open_checkout()
		_update_hud()
		return
	hud.hide_dialogue()
