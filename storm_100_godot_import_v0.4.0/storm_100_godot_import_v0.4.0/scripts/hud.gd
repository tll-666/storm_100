...
func _build_intro() -> void:
	...
	panel.add_child(box)
	var kicker := _make_label("GODOT正式工程 · 家庭生存底层 0.4.0", 13, Color("90bccc"))
	...
	var controls := _make_label(
		"WASD / 方向键：移动　　E：互动　　B：随身背包　　C：家庭状态\nEsc：关闭界面　　F5：保存检查点　　F9：读取检查点", 14, Color("aebcc1")
	)
	controls.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	box.add_child(controls)
	var note := _make_label("当前重点测试家庭生存底层；暴雨事件会在后续版本改变供水、供电和房间状态。", 13, Color("88979d"))
	box.add_child(note)
