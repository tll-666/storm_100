...
func _build_intro() -> void:
	...
	panel.add_child(box)
	var kicker := _make_label("GODOT正式工程 · 物品栏与通行修正 0.2.1", 13, Color("90bccc"))
	...
	var description := _make_label(
		"与四位家人交谈并检查房屋后，可以从车库开车去社区超市。你需要在220元和10格后备箱限制下完成第一次普通购物，再回家查看家人的简短反应。", 15, Color("cbd5d7")
	)
	description.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	description.custom_minimum_size.y = 78.0
	box.add_child(description)
	var controls := _make_label(
		"WASD / 方向键：移动　　E：互动　　B：随身背包\nEsc：关闭界面　　F5：保存检查点　　F9：读取检查点", 14, Color("aebcc1")
	)
	controls.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	box.add_child(controls)
	var note := _make_label("当前仍使用功能性家具和灰盒色块，重点测试购物选择和一天的闭环。", 13, Color("88979d"))
	box.add_child(note)
