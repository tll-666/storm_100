...
func _build_intro() -> void:
	...
	panel.add_child(box)
	var kicker := _make_label("GODOT正式工程 · 时间循环原型 0.3.0", 13, Color("90bccc"))
	...
	var description := _make_label(
		"完成第一次普通购物后，继续安排下午、收听傍晚天气信息并在主卧结束当天。夜间会进行一次家庭结算，再以日期卡进入第二天早晨。", 15, Color("cbd5d7")
	)
	...
	var note := _make_label("当前仍使用功能性家具和灰盒色块，重点测试一天如何推进和切换。", 13, Color("88979d"))
	box.add_child(note)
